...
async def run(project_state: dict) -> dict:
    ...
    if not media: project_state["edit_decision_list"] = []; return project_state
    audio = project_state.get("audio") or {}; audio_dur = float(audio.get("duration_sec") or 0.0); beats = _music_grid(audio)
    ...
    fixed_total = sum(max(0.5, float(m.get("trim_end_sec") - m.get("trim_start_sec"))) if m.get("type") == "video" and m.get("trim_start_sec") is not None and m.get("trim_end_sec") is not None else (0.0 if m.get("type") == "photo" else max(0.5, float(m.get("duration_sec") or 0.0))) for m in media)
    photo_count = sum(m.get("type") == "photo" for m in media); minimum_total = fixed_total + photo_count * PHOTO_MIN_SEC
    target_total = max(audio_dur, minimum_total) if audio_dur > 0 else 0.0
    durations = _initial_durations(media, audio, pref)
    if target_total > 0:
        durations = _fit_total(durations, media, target_total, pref); durations = _snap_to_beats(media, durations, beats, target_total)
        ...
        if abs(diff) > 0.01:
            ...
            if last_photo is not None: durations[last_photo] = round(_clamp(durations[last_photo] + diff, PHOTO_MIN_SEC, PHOTO_MAX_SEC), 3)
    rng = _rng(str(project_state.get("project_id", ""))); edl: list[dict[str, Any]] = []; cursor = 0.0; photo_index = 0; energy = list(audio.get("energy_curve") or [])
    for i, item in enumerate(media):
        duration = round(float(durations[i]), 3); v = _vision(item)
        if item.get("type") == "photo":
            item["duration_sec"] = duration; item["ai_duration_sec"] = duration; item["ai_edit_score"] = round(_photo_score(item, pref), 3)
            item["duration_source"] = "ai_music" if audio_dur > 0 else "vision"; item["music_sync"] = bool(beats); item["vision_ai_used"] = bool(v.get("ai_used", item.get("vision_ai_used", False)))
            item["people_count"] = int(v.get("people_count", item.get("people_count", 0)) or 0); item["importance_score"] = round(_photo_score(item, pref), 3)
            movement = _choose_movement(item, rng, photo_index); kb = _ken_burns_params(movement, rng); photo_index += 1
        else: kb = None
        transition_in = 0.0 if i == 0 or beats else round(_clamp(0.78 - 0.28 * _energy_at(energy, cursor), TRANS_MIN_SEC, TRANS_MAX_SEC), 3)
        edl.append({"media_id": item["id"], "start_sec_in_final_video": round(cursor, 3), "duration_sec": duration, "ken_burns": kb,
                    "transition_in": transition_in, "transition_out": transition_in if i < len(media) - 1 else 0.0,
        ...
        cursor += duration
        if transition_in > 0: cursor -= transition_in
    project_state["edit_decision_list"] = edl
    project_state["edit_summary"] = {
        "vision_photos": sum(1 for m in media if m.get("type") == "photo" and m.get("vision_ai_used")), "total_photos": photo_count,
        "music_synced": bool(beats), "bpm": audio.get("bpm", 0.0),
        "average_photo_score": round(sum(_photo_score(m, pref) for m in media if m.get("type") == "photo") / max(1, photo_count), 3),
        "story_chapters": len(project_state["story_chapters"]), "music_climax_sec": project_state["music_structure"].get("climax_sec"),
