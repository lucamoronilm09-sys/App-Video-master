...
Il contratto principale è `render_manifest`; `edl` resta come alias di compatibilità.
"""
...
def _quantize(sec: float, fps: int) -> float:
    return round(round(sec * fps) / fps, 3)


def _resolution(spec: dict[str, Any]) -> tuple[int, int]:
    try:
        w, h = str(spec.get("resolution", "1920x1080")).split("x", 1)
        w, h = int(w), int(h)
        if w <= 0 or h <= 0:
            raise ValueError
        return w, h
...
def _validate_kb(kb: Any, is_photo: bool) -> None:
    ...
    if any(key not in kb for key in required):
        missing = next(key for key in required if key not in kb)
        raise ValueError(f"ken_burns incompleto: manca {missing}")
    ...
    for axis in ("pan_x", "pan_y"):
        start, end = float(kb[f"{axis}_from"]), float(kb[f"{axis}_to"])
        if not (0.0 <= start <= 1.0 and 0.0 <= end <= 1.0):
            raise ValueError(f"{axis} fuori [0, 1]: {start}->{end}")
...
def _zoompan(kb: dict[str, Any], frames: int, w: int, h: int, fps: int) -> str:
    zf, zt = float(kb["zoom_from"]), float(kb["zoom_to"])
    z = f"{zf}" if frames <= 1 or zf == zt else f"{zf}+({zt}-{zf})*on/{frames - 1}"
...
async def run(project_state: dict) -> dict:
    ...
    for a, b in zip(edl, edl[1:]):
        if abs(float(a["transition_out"]) - float(b["transition_in"])) > 1e-6:
    ...
    for i, entry in enumerate(edl):
        ...
        actual_duration = round(frames / fps, 3)
    ...
    for i in range(1, len(edl)):
        ...
        if d <= 0:
            ...
            transitions.append({"index": i, "from_segment": i - 1, "to_segment": i,
                                "duration_sec": 0.0, "offset_sec": round(acc, 3), "cut": True})
            current = out
            acc += durations[i]
        else:
            offset = _quantize(acc - d, fps)
            out = f"xf{i}"
    ...
    args += ["-c:v", VCODEC_MAP[vcodec], "-preset", ENCODE_PRESET, "-crf", str(CRF_MAP[vcodec]),
             ...
             "-color_trc", "bt709", "-color_range", "tv", "-r", str(fps),
