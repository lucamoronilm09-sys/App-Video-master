...
export interface MediaItem {
  id: string; source: MediaSource; drive_file_id?: string; path: string; type: MediaType; orientation: Orientation;
  width: number; height: number; duration_sec: number; order_index: number; fit_mode?: FitMode; background_fill?: BackgroundFill;
  trim_start_sec?: number; trim_end_sec?: number; source_fps?: number; face_count?: number; people_count?: number;
  importance_score?: number; vision_ai_used?: boolean; scene_type?: string; ai_duration_sec?: number; ai_edit_score?: number; music_sync?: boolean;
}
...
export interface MusicStructure { duration_sec?: number; bpm?: number; energy?: string; sections?: { start_sec: number; end_sec: number; label: string; energy: number }[]; climax_sec?: number; }
export interface AIFeedback { rating: "up" | "down"; reasons: string[]; note: string; created_at: number; }
export interface ProjectState {
  schema_version: number; project_id: string; name: string; user_prompt: string; media: MediaItem[]; audio: AudioTrack; audio_tracks?: AudioTrack[];
  style_profile: string; output_spec: OutputSpec; edit_decision_list: EditEntry[]; story_chapters: StoryChapter[]; music_structure: MusicStructure;
  clip_overrides: Record<string, ClipOverride>; ai_feedback?: AIFeedback | null; ai_feedback_history?: AIFeedback[];
  render_manifest?: RenderManifest; qa_report?: QAReport; errors: ProjectError[]; pipeline_log: PipelineLogEntry[]; created_at: number; updated_at: number;
}
